# thdd-docs

> Thdd Documentation Site

A collection site for various documents and notes from Thdd.

[简体中文](README.md)

## 📚 Documentation List

| Document | Description |
|----------|-------------|
| [Thdd Bot Commands](docs/thbot-commands.md) | Thdd Bot command list, including group management, entertainment, AI and other features |
| [Nova Bot Commands](docs/nobot-commands.md) | Nova Bot command list, including group management, entertainment, AI and other features |

🔗 Online Access: [https://docs.th-dd.top](https://docs.th-dd.top)

## 🚀 Local Development

### Requirements

- Node.js 24
- pnpm 10 
- VitePress 2.0.0-alpha

## 🤝 Contributing

- 📝 Submit an [Issue](https://github.com/th-dd/thdd-docs/issues) to report problems or suggestions
- 🔧 Submit a [Pull Request](https://github.com/th-dd/thdd-docs/pulls) to improve content

## 📄 License

This project is open-sourced under the [MIT](LICENSE) license.

---

QQ Group: [1065333145](https://qm.qq.com/q/AtZk7q0y88)

Made with ❤️ by thdd
